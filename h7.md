...
- OS: Ubuntu 24.03.3 LTS x86_64
- CPU: InterlCore i3 8th Gen
- RAM: 8GB
- Disk: 256GB
...
- Managed by: virt-manager (powered by libvirt)
- OS: Kali GNU/Linux Rolling x84_64
...
- Side note: don't know how accurate this is tho!    
...
For the conversion, you simply change the name of the file to include the `cs8` suffix.     
`rtl_433` can then read the file.     
    
**Note:** The filename also needs to include *center frequency* and *sample rate*, using a simple underscore as the delimeter.     
So I changed the file name to `urhConverted_433.92M_2000k.cs8`.      
(The crucial part for this to work: `_433.92M_2000k.cs8`)     
...
I first thought it had something do with me using `pipx`, which apparently does the whole virtual environment isolation automatically for you. I figured I'd try creating it myself (the virtual environment), following the same kind of workflow Tero showed us in class.      
...
> <https://www.offensive-wireless.com/how-to-install-universal-radio-hacker/> Section: "Install URH Method 3 – From Source"     
...
$ source env/bin/activate
```
#### Install Cython and clone repo
```bash
$ pip install cython
$ git clone --depth=1 https://github.com/jopohl/urh/
```
(The `--system-site-packages` option gives the virtual environment access to global site-packages modules.)    
(git clone used with the  `--depth=1` flag only installs the latest version)    
...
- We get the info by selecting the the whole sample =  `Ctrl + a` inside the signal viewer.
- <img width="1253" height="425" alt="Screenshot from 2025-12-06 18-00-59" src="https://github.com/user-attachments/assets/154d51d0-7e54-4bdc-b0ee-aff3a5fa1bad" />    
...
We get the duration by selecting `Show data as: Bits`, and then from the bit stream, selecting a single bit.     
URH will then display us the value, which is: **524µs**      
(µs = microseconds)     
<img width="1253" height="425" alt="Screenshot from 2025-12-06 18-15-18" src="https://github.com/user-attachments/assets/87e1808c-d664-4d42-8bcf-944932da519a" />     
...
2.3s = 2,300,000 µs   
0.000524s = 524 µs     
That means: The bit we measured, can be sent around **4 390** times, during the acceleration from 0 to 100km/h by the fastest car in the world. Talk about fast huh.     
...
<img width="1620" height="746" alt="Screenshot from 2025-12-06 16-37-27" src="https://github.com/user-attachments/assets/ff1d1f80-79d8-4b25-90c0-1d92d317c800" />
