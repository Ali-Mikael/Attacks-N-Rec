...
```
This effectively starts the virtual switch. 
### Ryu?
...
- Funnily enough I studied this last week as part of a networking course.
...
```
Without googling and doing an educated guess here:
- `mn` probably = MiniNet
- `--topo single,3` = single topology & 3 hosts
- `--mac` = the mac flag probably says something like create mac addresses
- `--switch ovsk` = the type of switch to use?
- `--controller remote` = the one we started before running this command (using ryu)

Legit check (corrections):
- 
