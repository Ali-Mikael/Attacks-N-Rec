...
- The directory had some files and folders, but I ended up in the `/usr/share/nmap/nselib` directory, as the `nselib` most probably stands for **Nmap Scripting Engine Library**. (or something along those lines anyway)
...
- I could've also changed the user-agent line at the same time, but chose to keept it for now & just change it on the run with the command instead.
