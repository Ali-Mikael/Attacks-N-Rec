...
- The last piece of information tells us the user agent. Browser & OS and other details of the client etc..       
...
The results:
- By using Nmap with he `-A` flag, it runs multiple advanced detection features at once.
  - The `A` stands for `Agressive detection mode`
  - You can read more on flags here: <https://www.cbtnuggets.com/blog/certifications/security/nmap-flags-what-they-are-when-to-use-them>
...
- From `Network Distance: 0 hop` we can determine that the host we are scanning is on the same network as us. (duhhh it's localhost 😂)
...
And wouldnt you know, there was another version of the file. So I then searched through the file:
```
...
## E) Catching a criminal in action
**Capturing packets during a portscan**      
I captured around 8 seconds of traffic while port scanning, the capture contains 2332 packets in total.   
First we can see a bunch of attempts made, in order to gather intel on open ports:
...
- If we inspect the frames, we can see that each attempt has it's own destination port.
  - This is nmap in action probing for intel.
...
- Even though the 'nmap' string only appears once in this little summary, we can see that that it has been utilized based on the other `HTTP requests` as well.
- They are used by `nmap` trying to gather intelligence.
- Based on the wireshark `http header` summary from the capture, we find interesting information.

These are some of the `requests` used by `Nmap` while running scripts. 
- I have provided the `header` used and a `script` description.
- (the "quoted" descriptions are directly copied from <https://nmap.org>)
- **/sdk**
  - A SOAP request used to identify vmware vcenters servers, and retrieve their version information. (<https://infosecwriteups.com/evading-detection-while-using-nmap-69633df091f3>)
- **/robots.tx** 
  - Script: `http-git`.
...
- `-i` and `-w` flags are used to define **regex parameters**.
...
```
When you run nmap with these parameters, atleast the `user-agent` won't snitch on you anymore!        
...
- <img width="1035" height="376" alt="Screenshot 2025-11-02 at 22 29 29" src="https://github.com/user-attachments/assets/f0577f25-776e-44cd-9267-81e6de661b79" />
  - As we can see from the the `web server access logs`, we have succesfully spoofed a user agent, and it isn't snitching on us anymore.
  - If you're not really familiar with Nmap, only one entry is giving it away:
    ...
    - Stay tuned for the next section on now to eradicate this as well! ;)
...
## I) LowEr CHeCk
#### Objective: Locate the line of code that's responsible for printing 'nmap' and get rid of it
- **Step 1:** Navigate to the `/usr/share/nmap` directory.
- **Step 2:** Locate the file containing the code.
- The directory had some files and folders, but I ended up in the `/usr/share/nmap/nselib` directory, as the `nselib` most probably stands for **Nmap Scripting Engine Library**.
...
- It provided me with the file and the exact line, just like that.
- I also wanted to see what else im working with, so i executed the command:
```
$ grep -in nmap http.lua
```
- This so that I could see all other places were `'nmap'` is mentioned.
...
- **Step 3:** Eradicate the evidence.
- The file `http.lua` got some `sudoedit` action and I navigated to line: `2625`.
...
- NOTE: I could've also changed the user-agent line at the same time, but chose to keep it for now!       
...
- Peep out the **time of execution** `22:59` (this so that we don't get lost in the log entries)
...
### Result
The `nmap` string is nowhere to be found!      
