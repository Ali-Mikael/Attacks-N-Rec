...
- From the first section we can see (among other things) that the DNS-resolver for a certain host @ `192.168.122.7` is making a DNS query to aquire the address of a web-server.      
...
The address aquired => `139.162.131.217`, the host is now initiating a connection to the web-server.       
...
- **Hosts:** By quickly going through the capture, we can see atleast 5 hosts involved.
- **Size:** 283 frames
...
- So the NIC of the host `192.168.122.7` does in fact not have a brand, but is software based.
