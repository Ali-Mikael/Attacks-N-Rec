...
> Installing using the git repo: <https://github.com/jopohl/urh>     
> Trying this: <https://www.offensive-wireless.com/how-to-install-universal-radio-hacker/> Section: "Install URH Method 3 – From Source"     
...
# F) Overview
> Describe the sample in general: Duration, which frequency, when was the sample taken?
...
You can also confirm by looking at the sample, that we have in fact 3 distinct signals.     

# G) Bits
> Demodulate the signal, so that you get raw bits.
> What's the right modulation? How long is one raw bit in time? Describe this time by comparing it to something. (Many signals have line encoding, so for the final bits, these "raw bits" still need to be processed.)     
...
Atleast to my untrained eye, the `ASK` seems to be the right modulation in this scenario. It's the most consistent.     

### A raw bit in time (sounds poetic innit xd)
We zoom in on a bit, and mark it as closely as possible from beginning to end, and get a value: 533µs (microseconds)     
...
> 533 microseconds equals = 0.000533 seconds     
